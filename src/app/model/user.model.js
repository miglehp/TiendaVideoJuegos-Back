...
const updateById = (userId, { username, password, email, active, fecha_nacimiento, foto_perfil_url, validado }) => {
    return db.query(
        'update users set username = ?, password = ?, email = ?, active = ?, fecha_nacimiento = ?, foto_perfil_url = ?, validado = ? where id = ?', [username, password, email, active, fecha_nacimiento, foto_perfil_url, validado, userId]
    );
}
